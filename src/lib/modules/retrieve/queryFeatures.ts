...
export function expandQueryTokens(query: string, intent: QueryIntent): string[] {
  ...
  if (/(?:为什么|为何|原因|为啥|why).{0,12}(?:文档)?(?:切分|切片|chunk)|(?:文档)?(?:切分|切片|chunk).{0,12}(?:为什么|为何|原因|为啥|why)/i.test(query)) {
    expansions.push("文本切分", "检索精度", "提高检索精度", "上下文长度", "控制上下文长度");
  }

  if (/(?:资料库|知识库|索引).{0,8}(?:健康|状态|检查)|(?:健康|状态|检查).{0,8}(?:资料库|知识库|索引)|缺失源文件|嵌入是否就绪/.test(query)) {
    expansions.push("资料库健康", "健康检查", "缺失源文件", "索引状态", "嵌入", "页面", "查看");
  }

  if (/(?:高级配置|高级功能|多模块|模块协同|模块.{0,6}配置|一步步.{0,8}配置)/.test(query)) {
    expansions.push("高级配置", "高级功能", "多个模块", "模块协同", "独立手册", "手册", "背景说明");
  }
