...
| `library_tasks_recent.json` | Recent import/reindex task progress snapshots (ring buffer). Import tasks include structured `preflightSummary` counts, redacted preflight issues, and `repairAction` hints such as retry, reselect, convert format, OCR then reimport, reindex, or support-bundle export. Low-text-density PDFs appear as structured `pdf_ocr_recommended` warning issues. `currentFile` and structured issue `filePath` may be redacted. |
